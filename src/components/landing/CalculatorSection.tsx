...
export const CalculatorSection: React.FC = () => {
  ...
  return (
    <section id="calculator-section" className="ds-section bg-soft bg-grid-pattern border-b border-line scroll-mt-20">
      <div className="max-w-6xl mx-auto px-5 md:px-8 space-y-[64px]">

        {/* Section Heading */}
        <div className="text-center max-w-3xl mx-auto space-y-4">
          <span className="ds-badge px-3.5 py-1.5 text-xs bg-[rgba(255,199,66,.12)] text-[#B8860B] border border-[rgba(255,199,66,.2)]">
            ОНЛАЙН РОЗРАХУНОК
          </span>
        ...
        <div className="glass-panel-light !rounded-[32px] p-6 sm:p-10 grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-7 space-y-8 flex flex-col justify-center">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-2xl font-black font-mono text-emerald bg-[rgba(24,165,88,.1)] px-3 py-1 rounded-xl border border-[rgba(24,165,88,.2)]">
                  {kw} кВт
                </span>
              </div>
              <input
                ...
                className="w-full h-3 bg-line rounded-lg appearance-none cursor-pointer accent-emerald"
              />
              <div className="flex justify-between text-[11px] text-muted font-mono font-bold">
                <span>3 кВт</span>
                ...
                {[
                  ...
                  <button
                    ...
                    className={`p-3.5 rounded-2xl text-left text-xs font-bold transition-all border ${
                      type === t.key
                        ? 'bg-[rgba(24,165,88,.12)] text-body border-emerald'
                        : 'bg-white text-muted border-line hover:text-body hover:border-emerald/50'
                    }`}
                  >
                    {t.label}
                  </button>
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                className="w-full px-4 py-3 text-xs font-bold text-body rounded-[18px] border border-line bg-white focus:outline-none focus:border-emerald transition-colors"
              >
          ...
          <div className="lg:col-span-5 glass-panel-light !rounded-[32px] p-7 flex flex-col justify-between space-y-6 relative">
            <div className="space-y-6">
              <h4 className="text-xs font-extrabold uppercase tracking-widest text-muted flex items-center gap-2 border-b border-line pb-3">
              ...
              <div className="space-y-1">
                <div className="text-[11px] font-bold text-muted">{tr('calc_res_gen')}</div>
                <div className="text-2xl font-black font-mono text-body">≈ {baseYield.toLocaleString()} <span className="text-sm font-normal text-muted">кВт·год/рік</span></div>
              </div>

              {/* KPI 2 Savings */}
              <div className="space-y-1">
                <div className="text-[11px] font-bold text-muted">{tr('calc_res_save_uah')}</div>
                <div className="text-2xl font-black font-mono text-emerald">≈ {annualSavingsUah.toLocaleString()} <span className="text-sm font-normal text-muted">грн/рік</span></div>
                <div className="text-[10px] text-muted font-mono">(≈ ${annualIncomeUsd.toLocaleString()} / ≈ €{annualIncomeEur.toLocaleString()})</div>
              </div>

              {/* KPI 3 Approx cost */}
              <div className="space-y-1 pt-2 border-t border-line">
                <div className="text-[11px] font-bold text-muted">{tr('calc_res_cost')}</div>
                <div className="text-2xl font-black font-mono text-body">≈ ${approxCostUsd.toLocaleString()}</div>
              </div>

              {/* KPI 4 Payback — Yellow accent */}
              <div className="p-4 rounded-2xl bg-gradient-to-r from-[rgba(24,165,88,.12)] to-[rgba(255,199,66,.11)] border border-[rgba(24,165,88,.25)] space-y-1 text-center">
                <div className="text-[11px] font-extrabold uppercase tracking-wider text-emerald">{tr('calc_res_payback')}</div>
                <div className="text-3xl font-black font-mono text-body">
