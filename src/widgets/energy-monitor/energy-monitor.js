...
(function () {
  ...
  var STYLE =
    ':host{' +
      '--em-radius:26px;--em-pad:clamp(1.1rem,1.4vw + 1.4vh,2rem);' +
      '--em-gap:clamp(.7rem,1.6vh,1.1rem);--em-value-fs:clamp(2.2rem,min(5vw,6vh),3.4rem);--em-chart-h:clamp(70px,14vh,110px);' +
    ...
    '.title{font-family:var(--em-font-display);font-weight:700;font-size:1.2rem;margin:0;color:var(--em-text);}' +
    '.head-right{display:inline-flex;flex-direction:column;align-items:flex-end;gap:.4rem;}' +
    ...
    '.switch-label{font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.04em;}' +
    ...
    '.switch[aria-checked="true"] .switch-label{color:var(--em-text);}' +
    '.live{display:inline-flex;align-items:center;gap:.4rem;font-size:.78rem;font-weight:700;color:#5ee29a;text-transform:uppercase;letter-spacing:.05em;}' +
    ...
    '.chart{display:flex;align-items:flex-end;gap:6px;height:var(--em-chart-h);padding-top:1.05rem;overflow:visible;}' +
    '.bar{position:relative;flex:1;align-self:flex-end;height:var(--h);min-height:3px;border-radius:6px 6px 2px 2px;' +
    ...
    '.bar.on{background:linear-gradient(180deg,rgba(34,184,101,.95),rgba(15,122,64,.55));}' +
    '.bar.active{background:var(--em-grad-sun);box-shadow:0 0 16px rgba(255,178,0,.5);}' +
    ...
    '.stats{display:grid;grid-template-columns:repeat(3,1fr);gap:.6rem;padding-top:clamp(.45rem,1.2vh,.85rem);border-top:1px solid rgba(255,255,255,.12);}' +
    '.stats>div{display:flex;flex-direction:column;gap:.15rem;}' +
    '.stats strong{font-family:var(--em-font-display);font-size:1.02rem;color:var(--em-text);}' +
    '.stats span{font-size:.74rem;color:var(--em-muted);}' +
    /* yearly row */
    '.year{display:grid;grid-template-columns:repeat(3,1fr);gap:.5rem;padding-top:.55rem;border-top:1px solid rgba(255,255,255,.12);}' +
    '.year>div{display:flex;flex-direction:column;gap:.15rem;min-width:0;}' +
    '.year strong{font-family:var(--em-font-display);font-size:.98rem;color:var(--em-accent);white-space:nowrap;}' +
    '.year span{font-size:.7rem;color:var(--em-muted);}' +
    '.year .year-forecast,.year .year-saved-v{color:var(--em-muted-2);}' +
    /* monthly breakdown */
    '.months{padding-top:.55rem;border-top:1px solid rgba(255,255,255,.12);}' +
    '.months-title{font-size:.74rem;font-weight:700;color:var(--em-muted-2);text-transform:uppercase;letter-spacing:.04em;}' +
    '.months-chart{display:flex;align-items:flex-end;gap:3px;height:38px;padding-top:.85rem;margin-top:.35rem;overflow:visible;}' +
    '.m-bar{position:relative;flex:1;height:var(--h);min-height:2px;border-radius:3px 3px 1px 1px;background:linear-gradient(180deg,rgba(34,184,101,.5),rgba(15,122,64,.22));transition:background .3s,box-shadow .3s;}' +
    ...
    '.m-bar.passed{background:var(--em-grad-sun);}' +
    '.m-bar.sel{box-shadow:0 0 10px rgba(255,178,0,.45);}' +
    ...
    '@media (prefers-reduced-motion:reduce){.bar,.bar-val,.m-bar{transition:none;}.live i{animation:none;}}';
  ...
  class EnergyMonitor extends HTMLElement {
    ...
    _build() {
      ...
      this._el = {
        title: $('.title'), liveLabel: $('.live-label'), unit: $('.unit'),
        sw: $('.switch'), swLabel: $('.switch-label'),
      ...
      var self = this;
      this._el.sw.addEventListener('click', function () { self._setPlaying(self._playing !== true); });
      /* any click inside the block (except the switch itself) drops out of auto into manual mode.
      ...
      this._el.card.addEventListener('click', function (e) {
        ...
        if (self._playing) self._setPlaying(false);
      }, true);
      this._built = true;
    }
    ...
    _init() {
      ...
      el.liveLabel.textContent = c.onlineLabel;
      ...
      el.swLabel.textContent = c.autoLabel;
      el.sw.setAttribute('aria-label', c.autoLabel);
      this._playing = c.autoplay !== false;
      el.sw.setAttribute('aria-checked', String(this._playing));
      ...
      c.monthlyPerKw.forEach(function (val, m) {
        ...
        bar.className = 'm-bar';
        bar.style.setProperty('--h', Math.max((val / maxMonth) * 100, 2) + '%');
        var v = document.createElement('b'); v.className = 'm-bar-val';
        bar.appendChild(v);
        var pick = function () { self0._selectMonth(m); };
        bar.addEventListener('click', pick);             /* click / tap selects */
        bar.addEventListener('mouseenter', pick);        /* hover-scrub on desktop */
      ...
      this._bars = c.profile.map(function (frac) {
        var bar = document.createElement('span');
        bar.className = 'bar';
        bar.style.setProperty('--h', Math.max((frac / self._peak) * 100, 3) + '%');
    ...
    _renderTotals() {
      ...
      for (var m = 0; m < n; m++) {
        ...
        yearKwh += v;
        if (m <= this._selMonth) cumKwh += v;   /* Jan → selected month (inclusive) */
      }
    ...
    /* ---- manual month pick (only when АВТО is off) ---- */
    _selectMonth(m) {
      if (this._playing) return;            /* auto mode follows today */
      if (m === this._selMonth) return;
      this._selMonth = m;
      this._renderMonths();
      this._renderTotals();
    }
    ...
    /* ---- play/pause toggle: on = auto (loop + month = today), off = manual ---- */
    _setPlaying(on) {
      this._playing = !!on;
      this._el.sw.setAttribute('aria-checked', String(this._playing));
      this._clearTimer();
